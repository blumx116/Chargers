...
class AttentionModelWrapper(gym.core.ObservationWrapper):
    def __init__(self,
        ...
        n_stations: int = og_space['station_locations'].shape[0]
        max_cars: int = og_space['car_locs'].shape[0]

        self.observation_space: Space = Dict({
            'stations': Box(0, np.inf, (n_stations, station_dims)),
            'cars' : Box(0, np.inf, (max_cars, car_dims))
        })
